...
def test_build_provenance_is_bound_to_exact_release_subjects() -> None:
    ...
    reference = attest.get("uses")
    assert isinstance(reference, str)
    assert reference.startswith("actions/attest@")
    assert len(reference.removeprefix("actions/attest@")) == 40
    settings = attest.get("with")
    assert isinstance(settings, Mapping)
    assert settings.get("subject-checksums") == "dist/SHA256SUMS"

    verify = _run("Verify wheel and source archive provenance")
    assert "dist/*.whl dist/*.tar.gz" in verify
    assert 'gh attestation verify "$artifact"' in verify
    ...
    assert _step_index("Verify exact published assets") < _step_index(
        "Verify wheel and source archive provenance"
    )
